/**
 * @fileoverview Gift Finder Quiz — /gift-finder
 *
 * @description
 * A 4-step guided quiz that narrows down gift recommendations based on
 * recipient, budget, occasion, and interest. Uses resolveGiftQuery() to
 * build a Storefront search query, then navigates to /search?q=<query>.
 *
 * @route GET /gift-finder
 *
 * @features
 * - 4 steps: recipient → budget → occasion → interest
 * - Inverts selected tile (black bg, white text)
 * - Linear progress bar at top (foreground fill)
 * - Clean opacity crossfade between steps
 * - Results headline + CTA to /search
 *
 * @design
 * - Monochromatic: black / white / gray only — no color accents
 * - Full-page white canvas (100dvh minimum)
 * - Rectangular tile cards, sharp edges (rounded-none)
 * - Typography-forward, generous whitespace
 * - Progress line at top, square "Next" button
 *
 * @accessibility
 * - Keyboard-navigable tiles (role="radio" group)
 * - Touch targets ≥ 44×44px
 * - Focus rings respect user preference
 */
...
export const meta: Route.MetaFunction = ({matches}) => {
    ...
    return (
        getSeoMeta({
            ...
            description: `Not sure what to get? Answer four quick questions and we'll point you to the perfect gift.`,
            url: buildCanonicalUrl("/gift-finder", siteUrl)
        }) ?? []
    );
};
...
export default function GiftFinder() {
    ...
    /** Selected value for current step (before committing) */
    const [selected, setSelected] = useState<string | undefined>(undefined);
    /** Whether we are rendering the results screen */
    const [done, setDone] = useState(false);
    /** Opacity key — toggled to trigger crossfade transition */
    ...
    /** Crossfade helper — hides current content, runs cb, then shows next */
    ...
    /* ── Resolved result (only needed on results screen) */
